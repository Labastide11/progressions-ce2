(function(){
  ...
  /*
    V36.55 — suppression définitive de l'ancien bouton Photos Drive.
    Il peut encore être présent dans un ancien index.html mis en cache :
    on le retire dès le chargement de la page.
  */
  ...
  const writeAttendance=data=>localStorage.setItem(attendanceKey(),JSON.stringify({
    morning:Array.from(new Set(data.morning||[])),
    afternoon:Array.from(new Set(data.afternoon||[]))
  }));
  ...
  const genderClass=sexe=>{
    const value=norm(sexe);
    if(['fille','feminin','female','f'].includes(value)||value.startsWith('fill')||value.startsWith('femin'))return 'student-list-card--lavender';
    if(['garcon','masculin','male','m','g'].includes(value)||value.startsWith('garc')||value.startsWith('mascul'))return 'student-list-card--turquoise';
    return 'student-list-card--neutral';
  };
  ...
  function updateSummary(rows,absentSet){
    ...
    const presentEl=$('studentPresentCount');
    const absentEl=$('studentAbsentCount');
    const copyEl=$('studentCopyCount');
  ...
  function render(){
    ...
    if(syncInfo){
      ...
      syncInfo.textContent=hasDate
        ? 'Actualisé à '+date.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})
        : 'Jamais actualisé';

      syncInfo.title=hasDate
        ? 'Dernière actualisation : '+date.toLocaleString('fr-FR')
        : '';
    ...
    list.innerHTML=rows.map((s,index)=>{
      ...
      /*
        V36.55 — IMPORTANT :
        on transmet directement s.photo au module local.
        C'est cette correction qui permet d'afficher ce2-XX.png.
      */
      const portrait=
        window.ProgressionsStudentPhotos?.get?.(
          s.prenom,
          s.sexe||'',
      ...
      return `<article class="student-list-card ${cardClass}${cham?' is-cham':''}${ulis?' is-ulis':''}${absent?' is-absent':''}">
        ...
        <button class="student-list-card__portrait" type="button" data-attendance-key="${esc(key)}" aria-pressed="${absent?'true':'false'}" aria-label="${absent?'Remettre':'Marquer'} ${esc(s.prenom)} ${absent?'présent':'absent'} ${period}">
          <img src="${portrait}" alt="" onerror="window.ProgressionsStudentPhotos?.onError?.(this,'${esc(s.sexe||'')}')">
        </button>
        <div class="student-list-card__main">
          <div class="student-list-card__name">
            <strong>${esc(fullName)}</strong>
            <span class="student-list-card__markers">
              ${cham?'<span class="student-list-card__cham" title="Élève CHAM" aria-label="Élève CHAM">🎵</span>':''}
              ${ulis?'<span class="student-list-card__ulis" title="Élève ULIS" aria-label="Élève ULIS">⭐</span>':''}
            </span>
          </div>
    ...
    list.querySelectorAll('[data-attendance-key]').forEach(btn=>
      btn.addEventListener('click',()=>toggleAttendance(btn.dataset.attendanceKey))
    );
  }
  ...
  if(refreshBtn)refreshBtn.addEventListener('click',async()=>{
    ...
    try{
      await window.ProgressionsRoster?.refresh?.(true);
    }finally{
  ...
  if(configureBtn){
    configureBtn.addEventListener('click',()=>{
      window.ProgressionsRoster?.configure?.();
      render();
    });
  }

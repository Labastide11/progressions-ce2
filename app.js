(function(){
  ...
  function preciseSkillsFor(subject,period){const data=window.PROGRESSIONS[subject]||{};const annual=Array.isArray(data.annualCompetencies)?data.annualCompetencies:[];if(annual.length)return annual.map(s=>Object.assign({_period:'all'},s));const periods=period==='all'?['p1','p2','p3','p4','p5']:[period];const map={p1:'p1Competencies',p2:'p2Competencies',p3:'p3Competencies',p4:'p4Competencies',p5:'p5Competencies'};let result=[];periods.forEach((p,pi)=>{const list=data[map[p]]||[];if(list.length){result=result.concat(list.map(s=>Object.assign({_period:p},s)));return;}const rowPeriod=classes.indexOf(p);(data.rows||[]).forEach((row,ri)=>{const description=row[rowPeriod+1]||'';if(!description)return;result.push({code:`${subject.toUpperCase()}-${p.toUpperCase()}-${String(ri+1).padStart(2,'0')}`,domain:row[0],title:description.length>110?description.slice(0,107)+'…':description,jeSais:defaultRetention(row[0],description),proofs:[],lsu:row[0],_period:p});});});return result;}
  ...
  function currentSkills(){if(state.mode!=='suivi')return null;const data=window.PROGRESSIONS[state.subject];if(Array.isArray(data?.annualCompetencies)&&data.annualCompetencies.length)return data.annualCompetencies;const map={p1:'p1Competencies',p2:'p2Competencies',p3:'p3Competencies',p4:'p4Competencies',p5:'p5Competencies'};const key=map[state.period];return key?(data[key]||[]):null;}

...
function resetInventory(){
  ...
  rows.unshift({label:'Journées de présence enregistrées',key:'progressions_ce2_presences_*',keys:presenceKeys,present:presenceKeys.length>0,count:presenceKeys.length,bytes:presenceKeys.reduce((n,k)=>n+bytesForKey(k),0)});
  rows.push({label:'Suivi administratif de rentrée',key:'progressions_suivi_rentree_*',keys:rentreeKeys,present:rentreeKeys.length>0,count:rentreeKeys.length,bytes:rentreeKeys.reduce((n,k)=>n+bytesForKey(k),0)});
  return rows;
}
...
function renderLocal(remoteState){
  const local=localRoster(),meta=metaRows(),cham=meta.filter(x=>yes(x.cham)).length,ulis=meta.filter(x=>yes(x.ulis)).length,birthMissing=meta.filter(x=>!String(x.naissance||'').trim()).length;
...
function renderPreview(){
  const inv=resetInventory(),active=inv.filter(x=>x.present),bytes=active.reduce((n,x)=>n+x.bytes,0),local=localRoster(),meta=metaRows();const cmp=lastComparison;
  const preserved=[
    ['Progressions pédagogiques','progressions_ce2_suivi_v2',localStorage.getItem('progressions_ce2_suivi_v2')!==null?'présentes':'absentes'],
    ['URL de synchronisation API',API_URL_KEY,readText(API_URL_KEY)?'configurée':'absente'],
